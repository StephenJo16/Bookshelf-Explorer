...
const ProfileLayout = ({ auth, user, children }: PropsWithChildren<{ auth: User, user: User, children: ReactNode }>) => {
                ...
                <div className="max-w-6xl mx-auto py-6 px-6 lg:py-8 lg:px-16 bg-white shadow sm:rounded-lg">
                    <div className="flex sm:flex-row gap-1 flex-col justify-between items-start sm:gap-4 sm:justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            {auth.id === user.id ? (
                                <Link href={route("profile.edit")} className="bg-gray-600 py-1 px-2 text-sm border border-transparent rounded-md text-white hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 transition ease-in-out duration-150">Edit Profile</Link>
                            ) : (
                                isFollowed ? (
                                    <form onSubmit={unfollow}>
                                        <button type="submit" className="bg-gray-600 py-1 px-2 text-sm border border-transparent rounded-md text-white hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 transition ease-in-out duration-150" >Unfollow</button>
                                    </form>
                                ) : (
                                    <form onSubmit={follow}>
                                        <button type="submit" className="bg-gray-600 py-1 px-2 text-sm border border-transparent rounded-md text-white hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 transition ease-in-out duration-150" >Follow</button>
                                    </form>
                                )
                            )}
                    ...
                    <div className="-me-2 flex items-center sm:hidden mb-2">
                        ...
                        >
                            ...
                            {showingNavigationDropdown ? 'Close' : 'View Navi'}
                        </button>
                    </div>
